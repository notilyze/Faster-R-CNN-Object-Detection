...
# Change this environment variable if you want to rescale images that otherwise get too large for the PIL package
# that otherwise result into warnings.
Image.MAX_IMAGE_PIXELS = 244898889
...
class Slicer(object):
    ...
    def __slice_images(self, tile_size, tile_overlap, number_tiles):
        ...
        for file in sorted(glob.glob(self.IMG_SRC + "/*")):
            ...
            for tile in tiles:
                new_im = im.crop(tile)
                img_id_str = str('{}_{:06d}'.format(file_name.split("\\")[1],img_no))
                print('img_id_str: {}'.format(img_id_str))
                if len(self._ignore_tiles) != 0:
                    if img_id_str in self._ignore_tiles:
                        self._ignore_tiles.remove(img_id_str)
                        continue
                new_im.save('{}/{}.{}'.format(self.IMG_DST, img_id_str, file_type))
    ...
    def __slice_bboxes(self, tile_size, tile_overlap, number_tiles):
        ...
        for xml_file in sorted(glob.glob(self.ANN_SRC + '/*.xml')):
            ...
            for tile in tiles:
                img_no_str = '{}_{:06d}'.format(im_filename,img_no)
                ...
                for obj in objects:
                    ...
                    if points_info == Points.NONE:
                        if tile[1] > obj_lbl[1] and tile[3] < obj_lbl[3]:
                            if tile[0] <= obj_lbl[0] <= tile[2] or tile[0] <= obj_lbl[2] <= tile[2]:
                                new_lbl = (max(tile[0], obj_lbl[0]) - tile[0], 0,
                                           min(tile[2], obj_lbl[2]) - tile[0], tile_h)
                            else:
                                empty_count += 1
                                continue
                        elif tile[0] > obj_lbl[0] and tile[2] < obj_lbl[2]:
                            if tile[1] <= obj_lbl[1] <= tile[3] or tile[1] <= obj_lbl[3] <= tile[3]:
                                new_lbl = (0, max(tile[1], obj_lbl[1]) - tile[1],
                                           tile_w, min(tile[3], obj_lbl[3]) - tile[1])
                            else:
                                empty_count += 1
                                continue
                        else:           
    ...
    def resize_by_factor(self, resize_factor, resample=0):
        ...
        None
        """
        validate_resize_factor(frame=resize_factor)
    ...
    def __resize_images(self, new_size, resample, resize_factor):
        ...
        for file in sorted(glob.glob(self.IMG_SRC + "/*")):
            ...
            file_type = file.split('/')[-1].split('.')[-1].lower()
            SSID=re.search("[A-Za-z]{2}_S[0-9]{3}",file_name).group(0)
            factor=resize_factor.loc[resize_factor['SSID']==SSID]['Resize_Factor']
            if file_type not in IMG_FORMAT_LIST:
                continue
            im = Image.open(file)
            if factor is not None:
                new_size = [0, 0]
                new_size[0] = int(im.size[0] * factor)
                new_size[1] = int(im.size[1] * factor)
    ...
    def __resize_bboxes(self, new_size, resize_factor):
        ...
        for xml_file in sorted(glob.glob(self.ANN_SRC + '/*.xml')):
            ...
            im_w, im_h = int(root.find('size')[0].text), int(
                root.find('size')[1].text)
            im_filename = root.find('filename').text.split('.')[0]
            an_filename = xml_file.split('/')[-1].split('.')[0].split('\\')[1]
            SSID=re.search("\A[A-Za-z]{2}_S[0-9]{3}",im_filename).group(0)
            factor=resize_factor.loc[resize_factor['SSID']==SSID]['Resize_Factor']
            if factor is None:
                w_scale, h_scale = new_size[0]/im_w, new_size[1]/im_h
            else:
                w_scale, h_scale = factor, factor
                new_size = [0, 0]
