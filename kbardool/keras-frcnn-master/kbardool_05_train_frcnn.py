...
# Random seed for research purposes, to obtain the same first image slice during training
random.seed(1)
...
# In the following classifier_old is created to go from 2 classes (in pretrained weights) to an arbitrary new number of classes of classification
...
model_all_old = Model([img_input, roi_input], rpn[:2] + classifier_old)
print('\n\n\n\n')
...
plot_model(model_all, to_file=C.model_path[:-5]+'_model.png',show_shapes=True)
epoch_length = 2
		...
		try:
			...
			if iter_num == epoch_length:
				...
				if C.verbose:
					...
					print('Losses RPN cls: {}'.format(losses[:, 0]))
					print('Losses RPN regr: {}'.format(losses[:, 1]))
