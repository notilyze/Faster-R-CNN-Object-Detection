...
parser.add_option("--input_boost_alpha", dest="input_boost_alpha", help="Input path for boost importance alpha.")
parser.add_option("--input_boost_weight", dest="input_boost_weight", help="Input path for boost error weights.")
...
if not options.input_boost_alpha:
	parser.error('Error: path to boost alpha data must be specified. Pass --input_boost_alpha to command line')

if not options.input_boost_weight:
	parser.error('Error: path to boost weights data must be specified. Pass --input_boost_weight to command line')
...
alpha_t_old=read_pickle(options.input_boost_alpha)
alpha_t=np.append(alpha_t_old,0)
weight_dict=read_pickle(options.input_boost_weight)
	...
	while True:
		try:
			...
			if iter_num == epoch_length:
				...
				alpha_t[-1]=1/(loss_rpn_cls+loss_rpn_regr+0.3)
				sum_exponents=0
				sum_errors=0
