...
def validate_tile_size(tile_size, img_size=None):
    ...
    if img_size is None:
        ...
        if isinstance(img_size, tuple)==False:
            raise TypeError('Image size must be a tuple. The argument was of type {}'.format(type(img_size)))
...
def validate_resize_factor(frame):
    ...
    for i in range(len(frame)):
        factor=frame.loc[i]['Resize_Factor']
        print("Resize factor: "+str(factor))
        if factor <= 0:
            raise ValueError(
